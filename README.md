...
| `TERMPDF_MAX_CACHED_PAGES` | `32`                            | Max page bitmaps held in the kitty image registry (clamped 4–256). Default sized for the **bidirectional** working set: `viewport(2-3) + forward prefetch(8) + backward scrollback(8) + buffer(5)` ≈ 24, rounded to 32 for direction-flip headroom. The dominant reading pattern is "scroll forward, peek back, read forward again" — under-sizing this cap evicts the just-visited backward pages as forward prefetch fills, forcing pdfium re-renders on every direction flip. Decrease if memory-constrained. |
